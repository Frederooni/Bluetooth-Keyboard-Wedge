...
void setup(void)
{
  ...
  if (! ble.sendCommandCheckOK(F( "AT+GAPDEVNAME=EPX Bluetooth Keyboard" )) ) {
...
void getUserInput(char buffer[], uint8_t maxSize)
{
  ...
  do {
    last_count = 0;
    while (Serial.available() > 0 && Serial.available() <= maxSize) {
      byte num_serial = Serial.readBytes(buffer + count, maxSize);
      Serial.println(String("\nGot ") + num_serial + String(" bytes from Serial\n"));
      count += num_serial;
      last_count += num_serial;
      maxSize -= num_serial;
    }
    while (ble.available() && maxSize > 0) {
      int c = ble.read();
      buffer[count] = (char) c;
      count++;
      last_count++;
    ...
    delay(2);
  } while ((count == 0 || last_count > 0) && maxSize > 0);
}
